...
class Agent():
    ...
    def __init__(self,portStart,portEnd,nports,verbose=True):   
    ...
    def _package_action_into_msg(self,action):
        if(action==0):
            return srv.Msg(const.AMSG_SCAN,None)
        else:
            return srv.Msg(const.AMSG_EXPLORE,action)
    ...
    def _select_action(self):
        if (np.random.random() < self.eps):
            if (np.random.random() < self.beta):
                ...
                return np.random.randint(self.portStart,self.portEnd+1)        
        else:
